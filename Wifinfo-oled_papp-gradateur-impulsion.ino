...
/* ======================================================================
Function: _papp
Purpose : permit using papp in loop()
Input   : 
Output  : papp value
Comments: -
====================================================================== */
...
if (me && me->next) {
  while (me->next)
  {
    ...
    // BASE value
    if(!strcmp(me->name, "BASE")) base = String(me->value).substring(1).toInt();
    if(!strcmp(me->name, "EJPHN")) base = String(me->value).substring(1).toInt();
    
    // PAPP value
    if(!strcmp(me->name, "PAPP")) papp = String(me->value).substring(1).toInt();


   }

  //display.clear();
  display.drawString(0, 1, "Base: " + String(base) );
  display.drawString(0, 16, "Conso VA: " + String(papp) );
  //display.display();
...
// on envoie tout
//effacement de l'oled
display.clear();

   //état du bypass 
   //NO_Bypass = digitalRead(BYPASS);   
   adc2 = ( 1000 * analogRead(A0) / 1024 );
  if (adc2 >= 500) NO_Bypass = 1;
  else NO_Bypass = 0;
    //collecte de l'info PAPP dans la fonction _papp
   mypapp=_papp();
   
    
    //vérifie que le switch de bypass n'est pas actif
           if (NO_Bypass == 1)
                {
                premier_bypass = true;
                //modif luminosité LED
                // impulsion de ratio HAUT/BAS fonction de regul sur la broche de la led 
                // Si mypapp à 0 depuis plus de 10s augmentation du chauffe eau
                if(millis() - previousMillis >= 10000) 
                            {
                            // Garde en mémoire la valeur actuelle de millis()
                            previousMillis = millis();
                            if (mypapp == 0 ) regul = regul -10;
                            if (regul <= 300) regul = 300;
                            digitalWrite(UP, HIGH);
                            delay(100);
                            digitalWrite(UP, LOW);
                            delay(200);
            ...
            {
            regul = 300;
            if (premier_bypass == true)
                {
                  premier_bypass = false;
                  for (int i=0; i <= 15; i++)
                    { 
                    digitalWrite(UP, HIGH);
                    delay(100);
          ...
          uint8_t anotherflag = TINFO_FLAGS_NONE;
          ValueList * anotherme = tinfo.addCustomValue("Regul", buf, &anotherflag); //OK
